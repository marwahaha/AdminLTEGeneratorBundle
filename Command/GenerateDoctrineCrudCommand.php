...
namespace Donjohn\AdminLTEGeneratorBundle\Command;
...
use Sensio\Bundle\GeneratorBundle\Command\GenerateDoctrineCommand As BaseCommand;
...
use Donjohn\AdminLTEGeneratorBundle\Generator\DoctrineCrudGenerator;
use Donjohn\AdminLTEGeneratorBundle\Generator\DoctrineFormGenerator;
...
class GenerateDoctrineCrudCommand extends BaseCommand
{
    ...
    {
        $this
            ->setName('donjohn:generate:crud')
            ->setAliases(array('generate:donjohn:crud'))
            ->setDescription('Generates a AdminLTE theme CRUD based on a Doctrine entity')
            ->setDefinition(array(
                ...
                new InputOption('format', '', InputOption::VALUE_REQUIRED, 'The format used for configuration files (php, xml, yml, or annotation)', 'yml'),
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            ...
            $metadata = $this->getContainer()->get('doctrine')->getManager()->getClassMetadata($entityClass);
        } catch (\Exception $e) {
    ...
    {
        $questionHelper = $this->getQuestionHelper();
        $questionHelper->writeSection($output, 'Welcome to the AdminLTE theme Doctrine2 CRUD generator');
    ...
    {
        $this->getFormGenerator($bundle)->generate($bundle, $entity, $metadata, $forceOverwrite);
        $this->getFormGenerator($bundle)->generateFormFilter($bundle, $entity, $metadata, $forceOverwrite);
    }
